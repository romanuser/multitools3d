...
export function CadastroForm({ remainingPromoSlots }: { remainingPromoSlots: number | null }) {
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-paper px-4">
      <div className="w-full max-w-sm bg-surface border border-line rounded-2xl p-8">
        ...
        {remainingPromoSlots !== null && remainingPromoSlots > 0 && (
          <p className="mb-5 rounded-lg border border-amber/30 bg-amber-soft px-3 py-2.5 text-xs text-ink">
            🎁 Restam <strong className="font-semibold text-amber">{remainingPromoSlots}</strong>{" "}
            {remainingPromoSlots === 1 ? "vaga" : "vagas"} do bônus: quem se cadastrar agora ganha o
            plano <strong className="font-semibold">VIP vitalício</strong> de graça.
          </p>
        )}
